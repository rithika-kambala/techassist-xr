...
{
    [InitializeOnLoad]
    public static class BackendIntegrationSmokeTest
    {
        const string Key = "TechAssistXR.BackendSmokeRunning";
        static double deadline;
        static double nextActionTime;
        static int completed;
        static bool started;
        static int analysisPhase;
        static BackendIntegrationSmokeTest() { EditorApplication.update += Tick; }
        [MenuItem("TechAssist XR/Run Backend Integration Test")]
        ...
        static void Tick()
        {
            ...
            try {
                ...
                if (analysisPhase == 0) { analysisPhase = 1; diagnosis.ConfigureLive("http://127.0.0.1:8089", "integration-test-only-aaaaaaaaaaaaaaaa"); return; }
                if (analysisPhase == 1) {
                    if (diagnosis.Busy) return;
                    if (!diagnosis.IsLive) throw new Exception(diagnosis.Status);
                ...
                if (manager.Session.State == TechnicianState.SESSION_ENDED) {
                    ...
                    Finish(true, "Real HTTP backend, authenticated catalog, mock provider, persisted sessions and acknowledgments; 4/4 steps: Start/Complete button callbacks, wrong-target rejection, correct selection, guidance marker, acknowledgment and final state passed.");
                }
        ...
        static void Finish(bool success, string detail)
        {
            ...
            File.WriteAllText("Assets/TechAssistXRHybrid/Generated/BackendIntegrationTest.txt", report);
